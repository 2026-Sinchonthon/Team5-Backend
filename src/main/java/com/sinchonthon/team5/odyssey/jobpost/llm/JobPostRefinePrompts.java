...
final class JobPostRefinePrompts {
    ...
    static final String REVIEW_SYSTEM_PROMPT = """
            ...
            반드시 title과 refined_description 두 키만 가진 순수 JSON 객체를 출력한다. title은 구체적인 산출물이 드러나는 20자 이내 제목이다.
            """;
    ...
    static final String STRICT_REVIEW_SYSTEM_PROMPT = """
            ...
            반드시 title과 refined_description 두 키만 가진 순수 JSON 객체를 출력한다.
            """;
    ...
    private static final List<Map.Entry<String, String>> FEWSHOT = List.of(
            Map.entry(
                    ...
                    "{\"title\": \"미용실 예약 페이지\", "
                            + "\"refined_description\": \"인스타 프로필에서 연결되는 모바일 예약 페이지를 제작합니다. 고객이 날짜와 시간을 선택해 예약 요청을 제출하고, 매장에서 확인할 수 있도록 구성합니다. 온라인 결제는 요구되지 않았으며 예산은 200,000원입니다.\"}"
            ),
            Map.entry(
                    "[사장님 선택 카테고리: IMAGE]\n우리 빵집 로고랑 포장지에 넣을 스티커 디자인 감성있게 해줄 사람. 파일만 주면 됨. 8만원.",
                    "{\"title\": \"베이커리 로고 스티커\", "
                            + "\"refined_description\": \"빵집 로고와 포장용 스티커를 감성적인 방향으로 디자인하고 인쇄 가능한 원본 파일을 제공합니다. 최종 산출물은 이미지 파일이며 예산은 80,000원입니다.\"}"
            ),
            Map.entry(
                    "[사장님 선택 카테고리: SNS]\n우리 카페 인스타 계정에 올릴 짧은 영상 좀 찍고 편집해줄 사람 구해요. 유행하는 노래로. 다음 달 초까지.",
                    "{\"title\": \"카페 SNS 릴스\", "
                            + "\"refined_description\": \"카페 매장과 직원이 등장하는 짧은 영상을 촬영·편집해 인스타그램 릴스로 제작합니다. 게시 시점에 사용 가능한 음원을 선택하고 콘텐츠를 기획하며 마감은 다음 달 초입니다.\"}"
            )
    );
